...
export function VisitList() {
  ...
  if (visits.isLoading) return <div className="flex items-center justify-center p-8"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary" /></div>;
  if (!visits.data?.length) return <div className="text-center p-8 text-muted-foreground">Aucune visite</div>;

  return (
    <div className="p-4 space-y-3">
      <h2 className="text-xl font-bold">{isOwner ? 'Demandes de visite' : 'Mes visites'}</h2>
      {visits.data.map(visit => (
        <Card key={visit.id} className="p-4">
          ...
          {!isOwner && visit.status === 'pending' && visit.buyer_id === user?.id && (
            <Button size="sm" variant="outline" onClick={() => updateVisitStatus.mutate({ id: visit.id, status: 'cancelled' })}>
              <X className="h-4 w-4 mr-1" /> Annuler
            </Button>
          )}
        </Card>
